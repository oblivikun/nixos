...
spawnDynamicRofiTheme = do
 isDay <- io isDayTime
 when isDay $ spawn "rofi -theme ~/.config/rofi/config-light.rasi -show drun"
 when (not isDay) $ spawn "rofi -theme ~/.config/rofi/config-dark.rasi -show drun"
...
myStatusBarSpawner (S s) = do
 ...
 let statusBarConfig = statusBarPropTo ("_XMONAD_LOG_" ++ show s)
                                        ("xmobar" ++ show s ++ "~/.xmonad/xmobar" ++ (if isDay then "day" else "night") ++ ".hs")
